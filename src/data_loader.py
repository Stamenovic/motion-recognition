...
def load_vicon_csv(csv_path: Path) -> tuple[TrialMetadata, dict[str, SegmentTrajectory]]:
    ...
    Returns:
        (metadata, segments), segments keyed by segment name.
    """
    ...
    start_idx = next(
        (
            ...
            if row and row[0].strip() == "Segments"
        ),
        None,
    )
    ...
    # Columns per segment: rx, ry, rz, tx, ty, tz.
    segments = {
...
def load_trials(root: Path) -> list[TrialRecord]:
    ...
    for csv_path in iter_csv_paths(root):
        ...
        if parsed is None:
            logger.info("Skipping unlabeled CSV: %s", csv_path)
            continue
